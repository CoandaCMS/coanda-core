...
<div class="form-group @if (isset($invalid_fields[$attribute_identifier])) has-error @endif">
    ...
    <textarea class="form-control" id="attribute_{{ $attribute_identifier }}" name="attributes[{{ $attribute_identifier }}]" {{ (isset($attribute_definition['max_length']) ? 'maxlength="'.$attribute_definition['max_length'].'"' : '') }}>{{ ($old_input ? $old_input : $prefill_data) }}</textarea>
